<template lang="">
  ...
  <v-container>
    <v-row>
        <v-col>
            <v-btn v-if="showDel===false" class="text-body-2" @click="dialogPass=true" prepend-icon="mdi-key-alert" color="red">
                Aktifkan Mode Hapus
                <v-dialog v-model="dialogPass">
                    <v-card class="w-25 mx-auto pa-4">
                        <v-btn class="ml-auto mb-4" size="x-small" icon="mdi-close" color="red" @click="dialogPass=false"></v-btn>
                        <v-text-field v-model="inputtedPass" label="Masukkan Password" variant="outlined" density="compact"
                        :type="showPass?'text':'password'" :append-inner-icon="showPass ? 'mdi-eye' : 'mdi-eye-off'" @click:append-inner="showPass=!showPass"></v-text-field>
                        <div class="text-caption text-red mt-n4">{{passErrMess}}</div>
                        <v-card-action class="mx-auto">
                            <v-btn class="text-body-2" color="red" @click="activateDelete()">Aktifkan</v-btn>
                        </v-card-action>
                    </v-card>
                </v-dialog>
            </v-btn>
            <v-btn v-if="showDel===true" class="text-body-2" color="blue" prepend-icon="mdi-lock-check" @click="showDel=false">Nonaktifkan Mode Hapus</v-btn>
        </v-col>
        <v-col>
            <v-text-field class="w-75 ml-auto" v-model="searchValue" label="Cari Berdasarkan Nomor Nota" variant="outlined" density="compact"></v-text-field>
        </v-col>
    </v-row>
    <EasyDataTable :headers="headers" :items="items" theme-color="#AFB42B" table-class-name="custom-table" alternating :loading="loading" :search-field="searchField" :search-value="searchValue">
        ...
        <v-dialog v-if="showDel" v-model="dialogDel">
                <template v-slot:activator="{props}" v-slot:item.actions="{item}">
                    <v-btn class="text-body-2" prepend-icon="mdi-delete" variant="text" color="red" v-bind="props" @click="deleteId=item.id">Hapus</v-btn>
                </template>
                <v-card class="w-50 mx-auto text-center">
                    <v-card-text>Apakah anda yakin menghapus transaksi ini?</v-card-text>
                    <v-card-action class="my-4">
                        <v-btn color="blue" @click="delButton=true, deleteTrans()" :loading="delButton">Ya</v-btn>
                        <v-btn class="ml-16" color="red" @click="dialogDel=false">Tidak</v-btn>
                    </v-card-action>
                </v-card>
        </v-dialog>
        </template>
...
export default {
    ...
    methods: {
        ...
        deleteTrans() {
            axios.delete(useEnvStore().apiUrl + 'txs/' + this.deleteId)
                .then((res) => {
                    console.log(res)
