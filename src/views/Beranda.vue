<template>
  ...
  <v-container class="d-flex flex-row justify-center">
    <v-card class="w-100 mr-4" variant="tonal" color="deep-purple">
      <v-btn
        ...
        variant="icon"
        color="deep-purple"
        @click="(loading = true), getPenjualan()"
      ></v-btn>
  ...
  <v-btn
    ...
    position="fixed"
    class="mr-4 mb-4 text-body-1"
    color="teal"
    v-bind="props"
    @click="this.$router.push('/tambahtrans')"
...
export default {
  ...
  methods: {
    ...
    async getPenjualan() {
      try {
        const penjualan = await axios.post(
