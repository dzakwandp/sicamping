<template>
  ...
  >
    <v-btn
      ...
      @click="closeDialog()"
    ></v-btn>
    <div class="px-8 py-2 d-flex flex-row align-start mt-4">
      <vue-select
        class="w-50 mr-4"
        v-model="selected"
        :options="dataBarang"
            ...
            <div id="printContent">
              <v-container>
                ...
                <div class="text-total">
                  Total Harga: {{ formatCurrency(afterSubmit[0].total_harga) }}
                </div>
              </v-container>
            </div>
          </v-card>
...
export default {
  ...
  methods: {
    ...
    tambahItem() {
      ...
      const createIndex = this.totalItem.map((obj, index) => ({
        ...obj,
        index: index + 1,
      }));
      ...
      const total_bayar = this.totalItem.reduce(
    ...
    hapusItem(index) {
      ...
      const removedIndex = this.totalItem.filter(
